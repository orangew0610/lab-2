...
# ===---------------------------------------------------------------------------
#
# This is the entry of the FTM Compiler frontend.
#
...
class DynamoCompiler:
    """
    Dynamo Compiler is one of the frontends of FTM Compiler.
    Dynamo Compiler acts as a custom compiler for the TorchDynamo framework,
    which converts an FX Graph into an equivalent Graph and MLIR module.
    ...
    def __init__(
        ...
        Attributes:
            ...
            _imported_graphs: The graphs from dynamo importer.
    ...
    def imported_graphs(self):
        """Returns the imported graphs after compilation."""
    ...
    def _create_node(
        ...
        node_input: Tuple,
        node_users: List[str],
        node_output_shape: Optional[list] = None,
    ...
    def _compile_fx(
        ...
        """
        Compiles the provided FX Graph to Graph.
        ...
        if self._verbose:
            logger.debug("Graph in tabular form:")
...
# ---------------------------------------------------------------------------
# FTMCompiler — extends DynamoCompiler with FTM-specific compilation modes
# ---------------------------------------------------------------------------


class FTMCompiler(DynamoCompiler):
    """
    Compiler for FT-Matrix DSA processors.

    Extends DynamoCompiler with three compilation modes:
    1. compile_to_mlir() — capture PyTorch model, emit Linalg-level MLIR
    2. compile_to_lasm() — full pipeline: Linalg → ftm-opt → ftm-translate → llc → .lan
    3. jit_run()          — JIT execution via MLIR ExecutionEngine (CPU, for correctness)
    """

    def __init__(self, **kwargs):
        kwargs.setdefault("primary_registry", tosa_ops_registry)
    ...
    def compile_to_mlir(
        ...
        *args: torch.Tensor,
        ...
        """
        Capture a PyTorch model and lower to Linalg-level MLIR.

        Returns:
            List of Graph objects with _imported_module set at Linalg level.
        """
        self._imported_graphs = []
        self._imported_params = {}
        self.importer(model, *args)
        for graph in self._imported_graphs:
            graph.lower_to_linalg_ir()
        return self._imported_graphs
    ...
    def compile_to_lasm(
        ...
        *args: torch.Tensor,
        ...
        """
        Full FTM compilation pipeline:
            PyTorch → TOSA/Linalg MLIR → ftm-opt --ftm-pipeline
            → ftm-translate → llc -mtriple=matrix → .lan

        Args:
            model: PyTorch model.
            *args: Example input tensors.
            output_path: If provided, write first graph's LASM to this file.

        Returns:
            List of Graph objects (with _imported_module at Linalg level).
        """
        from .pipeline import FTMPipeline

        graphs = self.compile_to_mlir(model, *args)
        pipeline = FTMPipeline()

        for i, graph in enumerate(graphs):
            module = graph.get_mlir_module()
            if module is None:
                continue
            mlir_text = str(module)
            path = output_path if (output_path and i == 0) else None
            pipeline.run(mlir_text, output_path=path)
    ...
    def jit_run(
        ...
        *args: torch.Tensor,
        ...
        """
        JIT execution via MLIR ExecutionEngine on CPU.
        Useful for numerical correctness testing against PyTorch eager mode.
        ...
        # Compile the last graph and build an execution callable.
        graph = self._imported_graphs[-1]
        exec_fn = self._dynamo_run_for_graph(graph)

        # Collect all params + inputs in the order the graph expects.
        params_flat = self._imported_params.get(graph, [])

        return exec_fn(*params_flat, *args)


# Note: FFT generation helpers are provided by the `fftc` decorator module
# and `frontend/Python/transform/fft_builder.py`. A standalone helper
# `compile_fft_to_mlir` was previously added here for quick testing but
# is intentionally removed to keep `frontend.py` focused on the Dynamo/FTM
# compiler driver. Use `frontend/Python/fftc.py` (the `@algorithm` decorator)
# or call `frontend/Python/ops/fft.py::gen_dft_mlir_bindings` directly for
# text/bindings generation modes.
