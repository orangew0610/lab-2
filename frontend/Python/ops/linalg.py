...
def softmax_op(
    ...
    assert len(node.args) == 3
    if node.args[2]:
        raise NotImplementedError("softmax with half_to_full=True is not supported")
    input1 = symbol_table.get((str(node.args[0]), 0))
    dim = int(node.args[1])
    if input1 is None:
        return
...
def log_softmax_op(
    ...
    assert len(node.args) == 3
    if node.args[2]:
        raise NotImplementedError("log_softmax with half_to_full=True is not supported")
    input1 = symbol_table.get((str(node.args[0]), 0))
    dim = int(node.args[1])
    if input1 is None:
        return
...
def index_put_op(
    ...
    if num_vec_dims > 0:
        ...
        for i in range(len(input2)):
            ...
            except (ValueError, ir.MLIRError):
...
def scatter_add_op(
    ...
    if ir.RankedTensorType.isinstance(index_tensor.type):
