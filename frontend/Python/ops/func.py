...
# ===---------------------------------------------------------------------------
#
# The registry of mappings from graph node to MLIR func dialect operations.
#
# ===---------------------------------------------------------------------------
...
def func_op(node: FuncOp, symbol_table: Dict[Tuple[str, int], ir.Operation]):
    """
    Import the FuncOp.
    From FuncOp to MLIR FUNC Func operation.
    """
    arguments = []
...
def call_op(node: CallOp, symbol_table: Dict[Tuple[str, int], ir.Operation]):
    """
    Import the CallOp.
    From CallOp to MLIR FUNC call operation.
    """
    arguments = []
...
def call_external_op(
    ...
    """
    Import the CallExternalOp for external library calls (e.g., oneDNN).
